...
let group =  g.selectAll('path')
    .data(data, (d,i) => d.target.data.img_name)
    .join(
    function(enter){
...
function updateCircles(data){
    ...
    .on('click', async function(d){
           ...
           updatePathLinks = pathLinks.filter(function(item){
                   return !valueArray.includes(item.target.data.img_name);
           });
           ...
           updateCircleLinks = circleLinks.filter(function(item){
                    return !valueArray.includes(item.data.img_name);
           });
           ...
           var valueArray = [];
               ...
               return new Promise((resolve, reject)=>{
                     dlinks.forEach(async(element) =>{
                          valueArray.push(element.target.data.img_name);
                     });
                     resolve(valueArray);
               });
           }
      ...
      .join(
        function(enter){
            return enter.append('text')
                        ...
                        .text((d) => d.data.img_name + ':' + d.data.tag);
        },
        function(update){
            return update;
        },
